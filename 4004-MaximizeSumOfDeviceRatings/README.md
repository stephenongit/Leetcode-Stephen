...
**Difficulty:** Medium  
[View on LeetCode](https://leetcode.com/problems/maximize-sum-of-device-ratings/)

---

You are given a 2D integer array `units` of size `m &times; n` where `units[i][j]` represents the capacity of the `j^th` unit in the `i^th` device. Each device contains **exactly** `n` units.
...
- Device `0 = [3]`: `rating[0] = 3` - Device `1 = [2, 2, 1]`: `rating[1] = 1`
...
- Device `0 = [1, 2, 3, 4]`: `rating[0] = 1` - Device `1 = [5, 6]`: `rating[1] = 5`
